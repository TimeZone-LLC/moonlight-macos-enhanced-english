...
enum DebugLogParser {
    ...
    func flushPending(force: Bool, now: Date?) {
      ...
      case .hostIdentityMismatch:
        ...
        if current.certificateMismatchCount > 0 {
          details.append("Certificate mismatch: \(current.certificateMismatchCount) occurrences")
        }
        if current.incorrectHostCount > 0 {
          details.append("Incorrect host: \(current.incorrectHostCount) occurrences")
        }
  ...
  private static func defaultPresentation(
    ...
    case "discovery.mdns":
      if normalized == "starting discovery" {
        return .init(title: "Started scanning for hosts", detail: "Preparing to probe known hosts and local discovery services")
      }
      ...
      if let host = firstCapture(in: message, pattern: #"Found new host:\s*(.+)$"#) {
        return .init(title: "Discovered new host", detail: host)
      }
    ...
    case "pairing.identity":
      ...
      if normalized.contains("client certificate imported in memory without keychain access") {
        return .init(title: "Client certificate loaded into memory", detail: "This connection does not write to the keychain")
      }
      if let captures = captures(in: message, pattern: #"incorrect host:\s*([^\s]+)\s+expected:\s*([^\s]+)"#), captures.count >= 2 {
        ...
        return .init(title: "Response received from a host with a different identity", detail: "Actual \(actual) · Expected \(expected)")
      }
      return .init(title: cleaned, detail: nil)
    ...
    case "network.tls":
      ...
      return .init(title: "TLS / certificate validation failed", detail: cleaned)
    ...
    case "stream.lifecycle":
      ...
      if normalized.contains("stream target classification:") {
        ...
        return .init(title: "Stream route selected", detail: reason)
      }
      if let detail = suffix(in: cleaned, after: "Stream risk assessment:") {
        return .init(title: "Stream configuration assessment", detail: detail)
      }
      ...
      if normalized.contains("performclose invoked") {
        return .init(title: "User requested stream window close", detail: nil)
      }
    ...
    case "input.mouse":
      if normalized.contains("absolute pos=") {
        return .init(title: "Sending absolute mouse position", detail: cleaned)
      }
      if normalized.contains("relative raw=") {
        return .init(title: "Sending relative mouse movement", detail: cleaned)
      }
      return .init(title: cleaned, detail: nil)
    ...
    case "video.decoder":
      ...
      if normalized.contains("constructing new h264 format description") {
        return .init(title: "Rebuilding H.264 decoder format", detail: nil)
      }
    ...
    case "ui.window":
      ...
      if normalized.contains("window-did-enter-fullscreen") {
        return .init(title: "Window entered full screen", detail: nil)
      }
      if normalized.contains("window-did-exit-fullscreen") {
        return .init(title: "Window exited full screen", detail: nil)
      }
      return .init(title: cleaned, detail: nil)
  ...
  private static func friendlyNetworkFailureTitle(for code: String) -> String {
    switch code {
    case "-1202":
      return "Certificate validation failed; trying fallback"
    case "-1200":
      return "TLS connection failed; trying fallback"
    case "-1001":
      return "Request timed out; trying fallback"
    case "-1004":
      return "Cannot connect to server; trying fallback"
    case "-1005":
