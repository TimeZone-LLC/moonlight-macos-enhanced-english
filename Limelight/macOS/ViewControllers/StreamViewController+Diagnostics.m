...
- (void)scheduleConnectWatchdogCheckForToken:(NSInteger)token delay:(NSTimeInterval)delay {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (!connectionObjectReady) {
            ...
            NSString *timeoutMessage = @"The host is still starting or resuming the stream. This can take longer than video startup.\nKeep waiting, reconnect, or return and try again.";
        ...
        if (strongSelf.reconnectInProgress) {
            ...
            [strongSelf showErrorOverlayWithTitle:@"Reconnect Timed Out"
                                          message:@"Reconnecting took too long. The connection may have been lost.\nCheck your network or adjust the settings."
                                          canWait:NO];
            return;
        }
        ...
        if (!strongSelf.didAutoReconnectAfterTimeout &&
            ...
            [strongSelf showReconnectOverlayWithMessage:@"Network is not responding. Reconnecting…"];
        ...
        NSString *timeoutMessage = [strongSelf isAutomaticRecoveryModeEnabled]
            ? @"No video data has arrived for 15 seconds.\nCheck the network connection or try the options below."
...
- (void)showErrorOverlayWithTitle:(NSString *)title message:(NSString *)message canWait:(BOOL)canWait {
    ...
    if (!self.timeoutOverlayContainer) {
        ...
        NSButton *reconnectBtn = [NSButton buttonWithTitle:@"Reconnect" target:self action:@selector(handleTimeoutReconnect:)];
...
- (NSDictionary<NSString *, NSString *> *)compactPresentationForLogLine:(NSString *)rawLine {
    ...
    if ([line localizedCaseInsensitiveContainsString:@"[curated]"]
        ...
        return @{
            ...
            @"line": @"<WARN> [Log] Repeated log suppression summary"
        };
    }
        ...
        || [line localizedCaseInsensitiveContainsString:@"tcp_input"])
    {
        ...
        return @{
            @"key": [NSString stringWithFormat:@"noise.net.%@", code],
            @"line": [NSString stringWithFormat:@"<WARN> [Network Stack] Connection error %@", code]
        };
    }
    ...
    if ([line localizedCaseInsensitiveContainsString:@"Received response from incorrect host:"]) {
        return @{ @"key": @"default.identity.host", @"line": @"<WARN> [Identity] Response from incorrect host" };
    }
    ...
    if ([line localizedCaseInsensitiveContainsString:@"Stream target classification:"]) {
        return @{ @"key": @"default.stream.classification", @"line": @"<INFO> [Stream] Route selected" };
    }
...
- (void)updateLogOverlayToolbarState {
    ...
    if (statusLabel) {
        ...
        [parts addObject:[NSString stringWithFormat:@"Showing %lu lines / %lu raw entries",
        ...
        if (self.logOverlayPauseUpdates && self.logOverlayPausedRawLines.count > 0) {
            [parts addObject:[NSString stringWithFormat:@"Paused, %lu pending entries",
...
- (void)showLogOverlay {
    ...
    NSButton *clearBtn = [NSButton buttonWithTitle:@"Clear and Start Here" target:self action:@selector(handleLogOverlayClearFromNow:)];
    ...
    self.logOverlaySearchField.placeholderString = @"Search keywords / host / error code";
