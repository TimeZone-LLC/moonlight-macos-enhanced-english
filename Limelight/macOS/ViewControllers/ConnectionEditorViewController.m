...
- (void)loadView {
    ...
    NSTextField *title = [NSTextField labelWithString:[NSString stringWithFormat:@"Connections - %@", self.host.displayName ?: @"-"]];
    ...
    NSTextField *defaultLabel = [NSTextField labelWithString:@"Default Connection"];
    ...
    NSTextField *currentLabel = [NSTextField labelWithString:@"Current Connection"];
    ...
    self.addField.placeholderString = @"Add IP address, hostname, or IPv6 address (optional port)";
    [addStack addArrangedSubview:self.addField];

    NSButton *addButton = [NSButton buttonWithTitle:@"Add" target:self action:@selector(addEndpoint)];
    [addStack addArrangedSubview:addButton];

    self.removeButton = [NSButton buttonWithTitle:@"Remove" target:self action:@selector(removeSelectedEndpoint)];
    ...
    self.testAllButton = [NSButton buttonWithTitle:@"Test All" target:self action:@selector(testAllEndpoints)];
    [actions addArrangedSubview:self.testAllButton];

    NSButton *closeButton = [NSButton buttonWithTitle:@"Close" target:self action:@selector(closeSheet)];
...
- (void)reloadMethodPopups {
    ...
    [self.defaultPopup addItemWithTitle:@"No Default (Automatic)"];
    self.defaultPopup.lastItem.representedObject = @"__none__";

    [self.defaultPopup addItemWithTitle:@"Automatic"];
    self.defaultPopup.lastItem.representedObject = @"Auto";

    [self.currentPopup addItemWithTitle:@"Automatic"];
...
- (NSString *)statusTextForAddress:(NSString *)addr {
    ...
    if (effectiveState == 1) {
        if (latency && latency.intValue >= 0) {
            return [NSString stringWithFormat:@"Online (%d ms)", latency.intValue];
        }
        return @"Online";
    }
    if (effectiveState == 0) {
...
- (void)updateSummaryLabels {
    ...
    self.statusLabel.stringValue = [NSString stringWithFormat:@"Host status: %@", anyOnline ? @"Online" : @"Offline"];
    NSString *autoAddr = self.host.activeAddress ?: @"-";
    self.autoLabel.stringValue = [NSString stringWithFormat:@"Automatically selected: %@", autoAddr];
}
...
- (void)handleDoubleClick:(id)sender {
    ...
    alert.informativeText = @"Saving the address replaces this connection endpoint.";
