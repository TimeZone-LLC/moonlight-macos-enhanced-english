        ...
        if (now - last < kWarnRepeatSuppressWindowSec) {
            ...
            if (suppressed > 0) {
                summaryLine = FormatLogLine(LOG_W,
                                            [NSString stringWithFormat:@"[curated] %.1fs, repeated %ld times (last: %@)",
    ...
    switch (category) {
        case LoggerNoiseCategoryDiscoveryChatter:
            if ([sampleLine rangeOfString:@"Resolved address:" options:NSCaseInsensitiveSearch].location != NSNotFound) {
                summary = [NSString stringWithFormat:@"%@: %.0f seconds, %ld occurrences (%@ repeated address resolution)",
                           NoiseCategoryDisplayName(category),
                           kNoiseAggregationWindowSec,
                           (long)count,
                           discoveryHost.length > 0 ? discoveryHost : @"unknown"];
            } else {
                summary = [NSString stringWithFormat:@"%@: %.0f seconds, %ld occurrences (%@, %@)",
                           NoiseCategoryDisplayName(category),
                           kNoiseAggregationWindowSec,
                           (long)count,
                           discoveryHost.length > 0 ? discoveryHost : @"unknown",
        ...
        case LoggerNoiseCategoryHostIdentityMismatch: {
            ...
            if (certificateMismatchCount > 0) {
                [parts addObject:[NSString stringWithFormat:@"Certificate mismatch: %ld occurrences", (long)certificateMismatchCount]];
            }
            if (incorrectHostCount > 0) {
                [parts addObject:[NSString stringWithFormat:@"Incorrect host: %ld occurrences", (long)incorrectHostCount]];
            }
        ...
        default:
            summary = [NSString stringWithFormat:@"%@: %.0f seconds, %ld entries (main cause: %@, target %@)",
                       NoiseCategoryDisplayName(category),
                       kNoiseAggregationWindowSec,
                       (long)count,
